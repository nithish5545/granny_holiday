...
# ================= INIT DB =================
def init_db():
    # On hosted DBs (filess.io etc.), the database already exists and
    # CREATE DATABASE is not permitted. Connect directly using DB_CONFIG.
    try:
    ...
    try:
        ...
        print(f"[init_db] Setup failed: {e}")
    finally:
        conn.close()
...
try:
    ...
    print(f"[startup] init_db crashed: {e}")
